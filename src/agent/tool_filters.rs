...
//
// Generalized tool input filtering. Any tool can declare a "filterable field"
// and reuse the same deny/allow pattern infrastructure.
...
pub struct CompiledToolFilter {
    pub deny_patterns: Vec<(String, Regex)>,   // (original, compiled)
...
impl CompiledToolFilter {
    ...
    /// Deduplicates entries: hardcoded values that already exist (by pattern string
    /// or substring) are skipped.
    pub fn merge_hardcoded(
        mut self,
        deny_substrings: &[&str],
        deny_patterns: &[&str],
    ) -> Result<Self> {
        // Merge substrings (deduplicate by lowercase comparison)
...
mod tests {
    ...
    #[test]
    fn test_deny_substring_case_insensitive() {
        let f = make_filter(&[], &[], &["SUDO"]);
    ...
    fn test_deny_pattern_blocks() {
        ...
        assert!(f.check("sudo rm -rf /", "bash", "command").is_err());
        assert!(f.check("echo sudo", "bash", "command").is_ok()); // not at start
    }

    #[test]
    fn test_allow_pattern_restricts() {
    ...
    fn test_url_filtering() {
        ...
        assert!(f.check("http://example.com", "web_fetch", "url").is_err()); // not https
    }
