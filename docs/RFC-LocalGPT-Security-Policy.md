...
| `Unsigned` | Skip user policy. Warn: `"LocalGPT.md not signed. Run localgpt security sign."` | `unsigned` |
...
enum AuditAction {
    ...
    Signed,               // User ran `localgpt security sign`
...
| `signed` | User runs `localgpt security sign` | `"cli"` or `"gui"` |
...
| Source | Meaning |
|--------|---------|
| `"cli"` | User CLI command (`localgpt security sign`, `localgpt init`) |
...
Each entry hashes the previous entry, forming a tamper-evident chain. Broken chain → warning on `localgpt security audit`.
...
**Audit command tolerance:** `localgpt security audit` parses every line independently. Corrupted lines are reported inline, not treated as fatal errors:
...
```bash
localgpt security sign          # Sign LocalGPT.md
localgpt security verify        # Verify signature without starting a session
localgpt security audit         # Print audit log with chain validation
localgpt security audit --json  # Machine-readable output
localgpt security audit --filter write_blocked  # Show only specific action type
```
...
self.verified_security_policy = match &policy {
    ...
    PolicyVerification::Unsigned => {
        warn!("LocalGPT.md not signed. Run `localgpt security sign`.");
...
### 12.1 New Subcommand: `localgpt security`

```
localgpt security <COMMAND>
...
### 12.2 `localgpt security status` Output
...
| `test_audit_signed_on_cli` | `localgpt security sign` writes `signed` entry with source `"cli"`. |
...
| `test_audit_cli_reports_segments` | `localgpt security audit` reports chain segments and corrupted lines. |
...
9. **CLI commands** — `localgpt security sign|verify|audit|status`.
